...
/* Scenario: Food Order Processing
A food delivery app processes an order through a series of steps: preparing ingredients, cooking the food, packaging, and delivering it to the customer.*/

const foodOrderPipeline = [
  // Step:1 Prepared Ingredient
  (order) => {
    if (order.items && order.items.length > 0) {
      order.status = "Ingredient Prepared";
      console.log(`Ingredient Prepared for: ${order.items.join(", ")}`);
    } else {
      console.warn("No order found in given current Array");
      order.status = `Order Failed: No item in Given Array`;
    }
    return order;
  },
  // step: 2
  (order) => {
    if (order.status === "Ingredient Prepared") {
      order.status = "Cooking";
      console.log(`Cooking Started from: ${order.items.join(", ")}`);
    } else {
      console.log(`Cannot Start Cooking: Ingredient are not prepared`);
    }
    return order;
  },
  // step: 3
  (order) => {
    if (order.status === "Cooking") {
      ...
      console.log(`Packing Completed for: ${order.items.join(", ")}`);
    } else {
      console.log(
        "Cannot package food. Cooking is not completed Still it is proccing"
      );
    }
    return order;
  },
  // step 4: Delivery to customer
  (order) => {
    if (order.status === "Packaging") {
      order.status = "Out for Delivery";
      console.log(`Order is out for Delivery ${order.items.join(",")}`);
    } else {
      console.log(`cannot delivery food: Order is not Packaging`);
    }
    return order;
  },
];

let foodOrder = { items: ["Pizza", "Pasta"], status: "Order Placed" };
// foodOrder = foodOrderPipeline.reduce( ( order, stepFunc ) => stepFunc( order ), foodOrder );
// OR
// for(let i=0; i<foodOrderPipeline.length; i++){
//   foodOrder = foodOrderPipeline[i](foodOrder)
// }
// OR
for (const stepFunc of foodOrderPipeline) {
  foodOrder = stepFunc(foodOrder);
}
...
let studentEnrollment = {studentName:"sourabh",courseName:"Degital Marketing",
...
const orderPipeline = {
  ...
  applyDiscount: (order) => {
    if (order.totalPrice > 50) {
      order.discount = 10; // Flat discount for orders over $50
  ...
  // Calculate Tax
  calculateTax: (order) => {
    order.tax = order.totalPrice * 0.08; // 8% tax
...
// Example Usage:
let myOrder = { items: [], totalPrice: 0 };

// Add items to the order
myOrder = orderPipeline.addItem(myOrder, "Laptop");
myOrder = orderPipeline.addItem(myOrder, "Mouse");

// Update the total price
myOrder.totalPrice = 100;

// Apply discount, calculate tax, and generate summary
myOrder = orderPipeline.applyDiscount(myOrder);
myOrder = orderPipeline.calculateTax(myOrder);

// Generate and display summary
const summary = orderPipeline.generateSummary(myOrder);
